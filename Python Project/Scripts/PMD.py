...
def SwitchPokemon(turnlistinfo, movelistinfo, TestMode,pauseseconds,CompleteParticipatingPokemons):
    ...
    NewPokemon=NewPokemon[0]
    NewPokemon['PokemonTurnNumber']=0
    for x in range(len(CompleteParticipatingPokemons)):
        ...
        if CompleteParticipatingPokemons[x]['OwnerName']==Owner:
            ...
            ParticipatingPokemons_New.append(NewPokemon)
        else:
            ParticipatingPokemons_New.append(CompleteParticipatingPokemons[x])
    SwitchSQL = "EXEC dbo.Switch_Pokemon @SourcePokemonId = %s,@TargetPokemonId = %s,@BattleId = %s,@SelectTable = 0" %(MiniModules.ModifyValueForSQL(PokemonId),MiniModules.ModifyValueForSQL(NewPokemon['PokemonId']),MiniModules.ModifyValueForSQL(BattleId))
    if TestMode==1:
        print SwitchSQL
        pass
    MiniModules.runSQLNoResults(SwitchSQL,MiniModules.SpecialString(),1)
